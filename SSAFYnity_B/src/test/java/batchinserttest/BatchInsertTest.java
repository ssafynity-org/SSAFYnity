...
public class BatchInsertTest {
    ...
    private static final String[] CAMPUSES = {"서울캠퍼스", "대전캠퍼스", "부울경캠퍼스", "광주캠퍼스", "구미캠퍼스"};
    private static final String[] RANDOM_KOREAN_WORDS = {
            ...
            "인덱스", "조인", "트랜잭션", "커밋", "롤백", "세션", "JDBC", "MyBatis", "JPA", "ORM", "엔티티", "리포지토리",
    ...
    public void batchInsertTest() {
        ...
        int batchSize = 1000;
        int totalRecords = 13000000;

        for (int i = 2847013; i <= totalRecords; i++) {
            // 랜덤 Member 생성
            ...
            board.updateMember(member); // 연관관계 설정
            boards.add(board);
        ...
        System.out.println("총 " + totalRecords + "건의 데이터를 삽입했습니다.");
    }
    ...
    private String generateRandomKoreanName() {
        ...
        name.append(firstSyllables[random.nextInt(firstSyllables.length)]);
        name.append(middleAndLastSyllables[random.nextInt(middleAndLastSyllables.length)]);
        name.append(middleAndLastSyllables[random.nextInt(middleAndLastSyllables.length)]);
